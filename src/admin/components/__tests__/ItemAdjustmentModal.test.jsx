...
describe('ItemAdjustmentModal', () => {
  ...
  describe('Rendering', () => {
    test('should not render when isOpen is false', () => {
      render(
        <ItemAdjustmentModal {...defaultProps} isOpen={false} />
      );
  ...
  describe('Adjustment Type Toggle', () => {
    test('should toggle between Inventory Threshold and Item Details', async () => {
      ...
      const thresholdBtn = screen.getByRole('button', { name: /Inventory Threshold/i });
  ...
  describe('Form Input', () => {
    test('should update form fields on input change', async () => {
      ...
      expect(priceInput).toHaveValue(250.50);
    });
  });
  ...
  describe('Modal Actions', () => {
    ...
    test('should submit form with valid data', async () => {
      ...
      await user.selectOptions(
        screen.getByLabelText(/Item Type/i),
        'Uniform'
      );
