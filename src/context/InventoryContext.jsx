...
export const InventoryProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (user && [USER_ROLES.FINANCE, USER_ROLES.PSAS, USER_ROLES.ADMIN].includes(userRole)) {
      fetchInventory();
    }
  }, [user, userRole]);
  ...
  const addInventoryItem = async (itemData) => {
    try {
      ...
      const newItem = {
        id: `INV-${Date.now()}`,
        ...itemData,
        qrCode: `QR-${itemData.category.replace(/\s+/g, '-').toUpperCase()}-${itemData.size}-${Date.now()}`,
  ...
  const updateStock = async (itemId, newStock, reason = '') => {
    try {
      ...
      console.log(`Stock updated for ${item.name}: ${item.currentStock} -> ${newStock}. Reason: ${reason}`);
  ...
  const getInventoryStats = () => {
    ...
    const totalValue = state.items.reduce((sum, item) => sum + (item.currentStock * item.price), 0);
