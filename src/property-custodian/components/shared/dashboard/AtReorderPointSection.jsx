...
const AtReorderPointSection = ({ totalAtReorderPoint }) => {
  ...
  const { data, loading, error, refetch } = useInventoryAtReorderPoint(selectedEducationLevel);
  ...
  // Calculate total items (one row per size variant)
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <h2 className="text-lg sm:text-xl font-semibold text-[#0C2340]">
              At Reorder Point - {selectedEducationLevel} Level
            </h2>
